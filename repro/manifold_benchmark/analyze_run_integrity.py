...
def analyze(run_root: Path, output: Path, config_path: Path) -> dict[str, Any]:
    ...
    for record in records:
        ...
        checks = {
            ...
            "per_run_test_access_declared_after_its_training": bool(
                record.result.get("test_bank_accessed", False)
            ),
    ...
    payload = {
        ...
        "all_expected_jobs_in_launcher": launcher_jobs == set(expected_jobs(config)),
        ...
        "campaign_global_test_sequestering_satisfied": False,
        "campaign_global_test_sequestering_limitation": (
            "the frozen v1 runner evaluates test after each individual final "
        ...
        "analysis_test_access_policy": (
            "fresh task analysis remains blocked until all 36 final checkpoints pass integrity"
        ),
