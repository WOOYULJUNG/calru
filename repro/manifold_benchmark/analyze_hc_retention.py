...
def analyze_record(record, config, analysis_root: Path, device: torch.device):
    ...
    initial_model = build_topology_model(
        "hc", record.topology, model_seed=int(record.manifest["model_seed"])
    ).to(device)
    initial_model.eval()
...
def main() -> None:
    ...
    if args.job_id:
        ...
        if not records:
            raise ValueError("job is missing, not H-C, or not task-success eligible")
    for record in records:
