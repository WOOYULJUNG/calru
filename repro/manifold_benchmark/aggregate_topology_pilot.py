                ...
                paired.append(
                    {
                        ...
                        "baseline": baseline,
                        "seed": seed,
                        "hc_minus_baseline_test_error": _number(hc["test_mean_error"])
                        - _number(other["test_mean_error"]),
...
def orchestrate(args: argparse.Namespace) -> None:
    ...
    atomic_json(
        analysis_root / "analysis_launcher_manifest.json",
        {
            ...
            "wait_for_all_36_before_test_access": True,
        },
    )
    if args.wait:
        _wait_for_training(
    ...
    hc_jobs = [
        ...
        if record.model_id == "hc" and success.get(record.job_id, False)
    ]
    common = [
        "--run-root",
        str(run_root),
    ...
    _run(
        _module_command("plot_topology_analysis", "--analysis-root", str(analysis_root)),
        cwd=cwd,
    ...
    atomic_json(
        analysis_root / "ANALYSIS_COMPLETED.json",
        {
            ...
            "hc_task_success_runs": len(hc_jobs),
