...
def test_config_is_t128_source_q1_and_frozen_repair_contract() -> None:
    ...
    assert config["hyperparameter_tuning"]["learning_rate_grid"] == [
        0.03,
        0.01,
        0.003,
        0.001,
    ...
    assert config["hyperparameter_tuning"]["fanout_all_learning_rates"] is True
    ...
    assert paths["sagodi_rnn_tanh_n128"]["transition_noise"] == "models.py:333-349"
    assert paths["sagodi_rnn_tanh_n128"]["shipped_noise_config"] == (
        "params_ang_sparse.yml:13-14"
    )
    rnn = config["models"][0]
    ...
    assert [row["upstream_public_code_target_noise_std"] for row in config["models"]] == [
        0.0,
        0.01,
        0.01,
    ]
    assert {row["controlled_target_noise_std"] for row in config["models"]} == {0.0}
    assert {row["upstream_public_config_effective_state_noise_std"] for row in config["models"]} == {0.0}
    assert [
        row["upstream_public_config_nominal_state_noise_std"]
        for row in config["models"]
    ] == [0.0, None, None]
    assert [
        row["prior_internal_source_resolved_recipe_nominal_state_noise_std"]
        for row in config["models"]
    ] == [0.1, 0.0, 0.0]
    assert [
        row["prior_internal_source_resolved_recipe_effective_state_noise_std"]
        for row in config["models"]
    ] == [0.0316228, 0.0, 0.0]
    assert [row["upstream_public_code_output_dropout"] for row in config["models"]] == [
        0.0,
        0.5,
        0.0,
    ]
    assert {row["controlled_output_dropout"] for row in config["models"]} == {0.0}
...
def test_models_have_expected_counts_maps_bias_and_lstm_wd() -> None:
    ...
    for model_id in MODEL_IDS:
        model = build_model(config, model_id, 1e-3, 0.0)
        ...
        assert model.recipe.target_noise_std == 0.0
        assert model.recipe.output_dropout == 0.0
...
def test_noise_metadata_separates_nominal_and_actual() -> None:
    common = 0.0
    rnn = noise_metadata(MODEL_IDS[0], common)
    assert rnn["actual_post_transition_state_noise_std"] == common
    assert rnn["state_noise_scaling"] == "not_applicable_disabled"
    assert rnn["controlled_source_api_nominal_state_noise_std"] == 0.0
    assert rnn["paper_literal_state_noise_std_provenance_only"] == 0.1
    assert rnn["upstream_public_config_nominal_state_noise_std_provenance_only"] == 0.0
    assert rnn["upstream_public_config_effective_state_noise_std_provenance_only"] == 0.0
    assert rnn[
    ...
    assert rnn["hypothetical_nominal_0p1_effective_under_sqrt_dt_provenance_only"] == 0.0316228
    for model_id in MODEL_IDS[1:]:
        row = noise_metadata(model_id, common)
        assert row["actual_post_transition_state_noise_std"] == common
        assert row["controlled_source_api_nominal_state_noise_std"] is None
        assert row["controlled_source_api_nominal_semantics"] == (
            "not_applicable_no_upstream_state_noise_api"
        )
        assert row["upstream_public_config_effective_state_noise_std_provenance_only"] == 0.0
...
def test_bank_regeneration_tolerance_is_one_ulp_relative_not_loose_absolute() -> None:
    config = load_config()
    batch = source_angular_integration(
        16, 32999, stream_key=(CAMPAIGN_ID, "fixed_smoke_bank")
    )
    large = batch.latent_targets.clone()
    index = torch.argmax(large.abs()).item()
    flat = large.flatten()
    direction = torch.full_like(flat[index], float("inf"))
    flat[index] = torch.nextafter(flat[index], direction)
    one_ulp = Batch(
        inputs=batch.inputs,
        output_targets=batch.output_targets,
        latent_targets=large,
        mask=batch.mask,
        metadata=batch.metadata,
    )
    _validate_bank(one_ulp, trials=16, config=config, purpose="smoke")

    near_zero = batch.latent_targets.clone()
    near_index = torch.argmin(near_zero.abs()).item()
    near_zero.flatten()[near_index] += 2.1e-6
    beyond_absolute_tolerance = Batch(
        inputs=batch.inputs,
        output_targets=batch.output_targets,
        latent_targets=near_zero,
        mask=batch.mask,
        metadata=batch.metadata,
    )
    with pytest.raises(RuntimeError, match="latent_targets"):
        _validate_bank(
            beyond_absolute_tolerance, trials=16, config=config, purpose="smoke"
        )
...
def test_plan_counts_and_fresh_seed_boundaries(tmp_path: Path) -> None:
    ...
    assert {spec.actual_state_noise_std for spec in fanout} == {0.0}
...
def test_sentinel_and_fanout_never_select_missing_seed(tmp_path: Path) -> None:
    ...
    first_lr = config["hyperparameter_tuning"]["learning_rate_grid"][0]
    broken = next(
        spec
        for spec in fanout
        if spec.model_id == first_model
        and spec.learning_rate == first_lr
    )
    ...
    for learning_rate in config["hyperparameter_tuning"]["learning_rate_grid"]:
        target = next(
            spec
            for spec in fanout
            if spec.model_id == first_model
...
def test_worker_receipt_exact_set_and_checkpoint_identity(tmp_path: Path) -> None:
    ...
    assert manifest["initial_state_argument"] == "clean_post_update_q1_target"
    assert manifest["loss"] == "clean_all_step_masked_mse_on_cos_sin_targets"
    assert manifest["rng_stream_identities"]["target_noise"] == {
        "enabled": False,
        "generator_seed": None,
        "semantics": "clean_q1_initializer_and_clean_loss_target_no_rng_draws",
        "std": 0.0,
        "upstream_public_code_std_provenance_only": 0.0,
    }
    assert manifest["source_model_specific_training"]["controlled_target_noise_std"] == 0.0
    assert manifest["source_model_specific_training"]["controlled_output_dropout"] == 0.0
    assert manifest["rng_stream_identities"]["dropout"] == {
        "enabled": False,
        "global_torch_seed": None,
        "probability": 0.0,
        "semantics": "controlled_execution_has_no_dropout_rng_draws",
        "upstream_public_code_probability_provenance_only": 0.0,
    }
    ...
    assert manifest["rng_stream_identities"]["state_noise"]["scaling"] == (
...
def test_synthesized_failure_binds_noise_free_initial_and_rng_identity(
    tmp_path: Path,
) -> None:
    root = tmp_path / "failure-campaign"
    bank_path = root / "banks" / "smoke.npz"
    save_fixed_bank(
        bank_path,
    ...
    atomic_json(root / ROOT_MARKER, _scientific_identity(config_path, require_clean=False))
    (root / "inputs").mkdir(parents=True)
    shutil.copy2(config_path, root / "inputs" / config_path.name)
    spec = RunSpec(
        run_id="failure-smoke",
        stage="smoke",
        model_id=MODEL_IDS[0],
        model_seed=999,
        learning_rate=1e-3,
        ...
        batch_size=2,
        evaluation_bank=str(bank_path),
        output_dir=str(root / "smoke" / "runs" / "failure-smoke"),
        smoke=True,
    )
    _record_worker_failure(
        spec,
        config_path,
        "cpu",
        failure_kind="FloatingPointError",
    ...
    checkpoint["initial_state_dict_sha256"] = "0" * 64
    torch.save(checkpoint, checkpoint_path)
    receipt = strict_json_load(receipt_path)
    receipt["artifacts"][checkpoint_path.name] = sha256_file(checkpoint_path)
    atomic_json(receipt_path, receipt)
    assert not _verified_child(spec)

    checkpoint = deepcopy(original_checkpoint)
    checkpoint["rng_stream_identities"]["state_noise"]["enabled"] = True
    torch.save(checkpoint, checkpoint_path)
    receipt["artifacts"][checkpoint_path.name] = sha256_file(checkpoint_path)
    atomic_json(receipt_path, receipt)
    assert not _verified_child(spec)
