...
def test_hparam_analysis_contract_has_exact_selected_18_run_denominator():
    config = load_analysis_config(
        Path("repro/manifold_benchmark/topology_hparam_analysis_v1.json")
    )
    jobs = expected_jobs(config)
    assert config["expected_training"]["models"] == ["calru", "hc"]
    assert len(jobs) == 18
    assert len(set(jobs)) == 18
    assert config["execution"]["analyze_structure_for_all_runs"] is True
    assert config["expected_training"]["test_bank_was_sequestered_during_selection"] is True
